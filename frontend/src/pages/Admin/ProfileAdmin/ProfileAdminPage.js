...
function ProfileAdminPage() {
    ...
    const fetchUserInfo = async () => {
        ...
        try {
            ...
            setNotif({ open: true, type: 'error', title: 'Lỗi', message: e.message || 'Không thể tải thông tin', duration: 3000 });
        } finally {
            setLoading(false);
        }
    };
    ...
    const resolveActive = (u) => {
        ...
        if (typeof u.isActive === 'string') return u.isActive.toLowerCase() === 'true' || u.isActive === '1';
        if (typeof u.active === 'string') return u.active.toLowerCase() === 'true' || u.active === '1';
        return true;
    };
    ...
    const handleChangePassword = async () => {
        if (!changePasswordData.currentPassword || !changePasswordData.newPassword) {
            setNotif({ open: true, type: 'error', title: 'Thiếu thông tin', message: 'Vui lòng nhập đầy đủ thông tin', duration: 3000 });
            return;
        }
    ...
    return (
        <div className={cx('page')}>
                ...
                <div className={cx('profile-header')}>
                    <div className={cx('profile-left')}>
                        ...
                        <div className={cx('profile-info')}>
                            <h2 className={cx('name')}>{user.fullName || user.email || 'N/A'}</h2>
                            <p className={cx('role')}>Quản trị hệ thống</p>
                        </div>
                    </div>
                    <button className={cx('change-password-btn')} onClick={() => setShowChangePassword(true)}>
                        Đổi mật khẩu
                    </button>
                </div>
                ...
                <div className={cx('form-container')}>
                    <div className={cx('form-column')}>
                        ...
                        <div className={cx('form-group')}>
                            ...
                            <div className={cx('status-badge', isActive ? 'active' : 'locked')}>
            ...
            {showChangePassword && (
                <div className={cx('modal-overlay')} onClick={() => setShowChangePassword(false)}>
                    <div className={cx('modal')} onClick={(e) => e.stopPropagation()}>
                        <div className={cx('modal-header')}>
                            <h3 className={cx('modal-title')}>Đổi mật khẩu</h3>
                            <button className={cx('modal-close')} onClick={() => setShowChangePassword(false)}>×</button>
                        </div>
                        ...
                        <div className={cx('modal-footer')}>
                            <button className={cx('btn', 'cancel-btn')} onClick={() => setShowChangePassword(false)}>
                                Hủy
                            </button>
                            <button className={cx('btn', 'save-btn')} onClick={handleChangePassword}>
                                Đổi mật khẩu
                            </button>
                        </div>
                    </div>
