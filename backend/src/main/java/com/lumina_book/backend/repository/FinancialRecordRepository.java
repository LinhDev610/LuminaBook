...
public interface FinancialRecordRepository extends JpaRepository<FinancialRecord, String> {
    ...
    @Query("select date(fr.occurredAt) as d, sum(fr.amount) as total " +
           "from FinancialRecord fr " +
           "where fr.recordType = :type and fr.occurredAt between :start and :end " +
           "group by date(fr.occurredAt) order by d")
    List<Object[]> revenueByDay(@Param("type") FinancialRecordType type,
                                @Param("start") LocalDateTime start,
                                @Param("end") LocalDateTime end);

    @Query("select fr.product.id, fr.product.name, sum(fr.amount) " +
           "from FinancialRecord fr " +
    ...
    List<Object[]> revenueByProduct(@Param("type") FinancialRecordType type,
                                    @Param("start") LocalDateTime start,
                                    @Param("end") LocalDateTime end);
    ...
    List<Object[]> revenueByPayment(@Param("type") FinancialRecordType type,
                                    @Param("start") LocalDateTime start,
                                    @Param("end") LocalDateTime end);
}
