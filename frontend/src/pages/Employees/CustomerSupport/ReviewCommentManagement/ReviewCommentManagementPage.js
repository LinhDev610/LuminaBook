...
import { getAllReviews, replyToReview, getStoredToken, getUserById } from '../../../../services';
...
export default function ReviewCommentManagementPage() {
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try {
                ...
                if (!token) {
                    notifyError('Vui lòng đăng nhập để xem đánh giá.');
                    setLoading(false);
                    return;
                }
    ...
    const handleCustomerNameClick = async (userId) => {
        ...
        try {
            ...
            const token = getStoredToken('token');
            if (!token) {
                notifyError('Vui lòng đăng nhập để xem thông tin khách hàng.');
                return;
            }
    ...
    const handleSendReply = async (id) => {
        ...
        try {
            ...
            const token = getStoredToken('token');
            if (!token) {
                notifyError('Vui lòng đăng nhập để gửi phản hồi.');
                return;
            }

            const { ok, status, data } = await replyToReview(
                id,
                { reply: draft },
                token,
            );
