...
export default function RegisterModal({ open = false, onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        setError('');
        try {
            const payload = { username: (email || '').trim(), password, firstName: (username || '').trim() };
            ...
            <div className="auth-card">
                ...
                {step === 1 ? (
                    <form className="auth-form" onSubmit={handleSendEmail}>
                        ...
                        <button className="auth-submit" type="submit" disabled={isLoading}>{isLoading ? 'Đang gửi...' : 'Gửi mã xác nhận'}</button>
                    </form>
                ) : (
                    <form className="auth-form" onSubmit={handleSubmit}>
                        ...
                        <div className="form-group">
                            <label className="agree"><input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} /> Tôi đồng ý với điều khoản</label>
                        </div>
                        {error && <div className="error-text">{error}</div>}
                        <button className="auth-submit" type="submit" disabled={isLoading}>{isLoading ? 'Đang xử lý...' : 'Đăng ký'}</button>
                    </form>
                )}
            </div>
        </div>
