...
public class CartService {
    ...
    private double calculateUnitPrice(Product product) {
        ...
        var promosByCategory = product.getCategory() == null ? java.util.List.<com.lumina_book.backend.entity.Promotion>of()
                : promotionRepository.findActiveByCategoryId(product.getCategory().getId(), today);
        ...
        // Apply tax if provided (assume tax is percentage, e.g., 0.1 for 10%) to derive pre-tax unit price? Keep unitPrice pre-tax
        return discounted;
    }

    private void recalcCartTotals(Cart cart) {
        double subtotal = cart.getCartItems() == null ? 0.0 : cart.getCartItems().stream()
                .mapToDouble(CartItem::getFinalPrice).sum();
